(function() {
    ...
    var rx_one = /^[\],:{}\s]*$/,
      ...
      rx_dangerous = /[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g;
    var noop = function() {},
      _log_support = (typeof console != 'undefined' && typeof console.log == 'function'),
      log = function(a) {
        if (_log_support) {
          try { console.log(a) }
          catch (ex) {}
      ...
      logError = function(a) {
        if (_log_support) {
          try { isFunction(console.error) ? console.error(a) : console.log(a) }
          catch (ex) {}
        }
      },
      logDebug = function(a) {
        if (_log_support) {
          try { isFunction(console.debug) ? console.debug(arguments) : console.log(arguments) }
          catch (ex) {}
        }
      },
      ...
      _bind = function(func, ctx) {
        if (!isFunction(func)) return noop;
        var ntvBind = Function.prototype.bind;
        if (ntvBind && func.bind === ntvBind) {
          return ntvBind.apply(func, [].slice.call(arguments, 1));
        }
    ...
    function sendDefault(parameters, transportAddress, payload, eventType, context, callback) {
      callback = callback || noop;
      if (2036 >= payload.length) sendWithImage(parameters, transportAddress, payload, eventType, context, callback);
      else sendWithBeacon(parameters, transportAddress, payload, eventType, context, callback) || sendWithXHR(parameters, transportAddress, payload, eventType, context, callback) || sendWithImage(parameters, transportAddress, payload, eventType, context, callback);
    }
    
    function sendWithImage(parameters, transportAddress, payload, eventType, context, callback) {
    ...
    function sendWithBeacon(parameters, transportAddress, payload, eventType, context, callback) {
      if (parameters && parameters.get(_test_mode)) {
    ...
    function sendWithXHR(parameters, transportAddress, payload, eventType, context, callback, retries) {
      var isRetrieve = 'retrieve' == eventType;
      if (parameters && parameters.get(_test_mode)) {
        log('sendWithXHR: ' + eventType + ': ' + payload);
        if (parameters.get(_log_only) && !isRetrieve) return (callback(null, eventType, context), !0);
      }
      if (!window.XMLHttpRequest) return !1;
      var xhr = new window.XMLHttpRequest;
      ...
      retries = retries || 0;
      var err, started, dt, du, ab, uuid,
        response = null,
        handleError = function(msg) {
          msg = msg || response || (xhr && xhr.statusText);
          trigger('error', msg, eventType, context);
          if (!parameters.get(_silent)) logError(msg);
          if (xhr.retry) sendWithXHR(parameters, transportAddress, payload, eventType, context, callback, retries + 1);
          else callback(msg, eventType, context, xhr);
        };
        
      xhr.open("POST", transportAddress, !0);
      xhr.withCredentials = !0;
      xhr.timeout = parameters.getNumber(_timeout) || 5000;
      xhr.setRequestHeader("Content-Type", "text/plain");
      xhr.onreadystatechange = function() {
        if (xhr.readyState !== 4) return;
        if (started) parameters.set(_sending_count, parameters.getNumber(_sending_count) - 1);
        parameters.set(_sending, parameters.getNumber(_sending_count) > 0);
        response = xhr.responseText;
        if (xhr.getResponseHeader("content-type") && ~xhr.getResponseHeader("content-type").indexOf("application/json")) {
          try { xhr.responseJSON = response = JSON.parse(response) }
          catch (e) { response = xhr.responseText }
        }
      };
      xhr.onloadstart = function() {
        started = !0;
        parameters.set(_sending_count, parameters.getNumber(_sending_count) + 1);
        parameters.set(_sending, !0);
        trigger('sending', eventType, parameters.getParams(), context, payload);
      };
      xhr.onload = function() {
        if (xhr.readyState !== 4) return;
        ...
        try {
          dt = xhr.getResponseHeader('X-Certification-Token');
          du = xhr.getResponseHeader('X-Download-URL');
          ab = xhr.getResponseHeader('X-Activity-Batch');
          uuid = xhr.getResponseHeader('X-Signer-UUID');
          if (dt) parameters.set(_download_token, dt);
          if (du) parameters.set(_download_url, du);
          if (ab) parameters.set(_batch, ab);
          if (uuid && parameters.get(_uuid) != uuid) generateUUID(parameters, uuid);
        }
        catch (e) {}
        callback(null, eventType, context, xhr, ab);
      };
      xhr.onerror = function() {
        err = null;
        if (xhr.readyState === 4 && xhr.status === 0) err = 'Send encountered a network error';
        handleError(err);
      };
      xhr.onabort = function() {
        handleError('Send aborted by browser');
      };
      xhr.ontimeout = function() {
        if (retries < parameters.getNumber(_retry_attempts)) xhr.retry = !0;
        handleError('Send timed out');
      };
      xhr.send(payload);
      return !0;
    }
    ...
    TaskStore.prototype.executeTasks = function(parameters) {
      ...
      try {
        ...
        e = err;
        var msg = 'Command aborted on: ' + f + ', Error: ' + (err && err.name ? (err.name + ': ' + err.message) : (err || ''));
    ...
    function buildAction(parameters) {
      ...
      parameters.set(_event_payload, parameters.getPayload(), !0);
    }
    ...
    var getParameterValue = function(parameters, name) {
      ...
      setParameterValue = function(params, name, value, override) {
        if (void 0 != value) {
          switch (name) {
            case _site_id:
    ...
    ParameterStore.prototype.getNumber = function(name) {
      return getParameterNumber(this, name);
    };
    ...
    ParameterStore.prototype.extend = function(name, value, override) {
      if (name) {
        var current = getParameterValue(this, name);
        if (!current || "object" != typeof current) current = {};
        if (value && "object" == typeof value) {
          for (var key in value) {
            if (value.hasOwnProperty(key)) current[key] = value[key];
          }
          setParameterValue(this, name, current, override);
        }
      }
    };
    
    ParameterStore.prototype.getParams = function(overrides) {
      var self = this, val, data = {};
      if (overrides) this.set(overrides, void 0, !0);
      ParameterDefinitionMap.map(function(key, paramDef) {
        if (paramDef.shortName) {
          val = self.get(key);
          if (void 0 != val) data[key] = val;
        }
      });
      if (overrides) this.data.overrides = {};
      return data;
    };
    
    ParameterStore.prototype.getPayload = function(overrides) {
      var self = this, val, data = [];
      if (overrides) this.set(overrides, void 0, !0);
      ParameterDefinitionMap.map(function(key, paramDef) {
        if (paramDef.shortName) {
          val = self.get(key);
          void 0 != val && val != paramDef.defaultValue && ("boolean" == typeof val && (val *= 1), data.push(paramDef.shortName + "=" + tryEncodeURIComponent(val)));
        }
      });
      if (overrides) this.data.overrides = {};
      data.push("nc=" + generateCacheBuster());
      return data.join("&");
    };
    ...
    var _site_id = defineParameter("site_id", "sid"),
      ...
      _timeout = defineParameter("timeout", void 0, 5000),
    ...
    var _events_called = defineParameter("_ec", void 0, 0),
      ...
      _batch = defineParameter("batch"),
      _download_token = defineParameter("download_token"),
      _download_url = defineParameter("download_url", void 0, void 0, function(p, n, v) {
        if (!v && p.context && p.context.site) {
          v = p.context.site.get(n);
          if (v) v += '&gid=' + p.get(_group);
        }
        return v;
      });
    ...
    function defineMethod(methodName, obj, actualMethod) {
      obj[methodName] = function() {
        try { return actualMethod.apply(this, arguments) }
          ...
          trigger('error', msg, methodName, obj);
          logError(err);
          throw beaconSendError(null, "exec", methodName, msg), err;
        }
      }
    };
    ...
    GroupStore.prototype.add = function(site, key, type, options, callback) {
      ...
      if (obj) {
        ...
        if (typeof site != 'undefined' && !site.group) site.group = obj;
    ...
    var Site = function(options) {
      ...
      setParameter(_silent, options[_silent]);
    ...
    Site.prototype.extend = function(name, value) {
      this.parameters.extend(name, value);
    };
    
    Site.prototype.getParams = function(overrides) {
      return this.parameters.getParams(overrides);
    };
    
    Site.prototype.getPayload = function(overrides) {
      return this.parameters.getPayload(overrides);
    };
    ...
    Site.prototype.load = function(a) {
      if (!(1 > arguments.length)) {
        var opts, cb, val, host, prefix, fname, loc, dyn, isFilter;
        
        var groupName = arguments[0];
        if (!isString(groupName)) groupName = null;
        
        if (arguments[0] && 'object' == typeof arguments[0] && arguments[0].hasOwnProperty('filter')) {
          isFilter = !0;
        }
        else if (arguments.length > 1 && arguments[1] && 'object' == typeof arguments[1] && arguments[1].hasOwnProperty('filter')) {
          isFilter = !0;
        }
        
        if (isFilter || !isBlank(groupName)) {
          if (arguments.length == 3 && isFunction(arguments[2])) {
            opts = toObject(loadCommandOptions, [arguments[1]]);
            opts[_event_callback] = arguments[2];
          }
          else if (isFilter && arguments.length == 1) opts = toObject(loadCommandOptions, [].slice.call(arguments, 0));
          else opts = toObject(loadCommandOptions, [].slice.call(arguments, 1));

          if (!groupName) {
            groupName = 'group-' + (new Date).getTime().toString();
            opts[_virtual] = !0;
          }
          opts[_key] = groupName;
          ...
          if (!this.groups.getLoading(groupName)) {
            ...
            dyn = this.get(_dynamic) || isFilter;
    ...
    var wrapCallback = function(callback, overrides) {
      return function(err, event_type, context, payload, batch) {
        callback(err, event_type, context, payload, batch);
        if (!err) trigger('sent', event_type, { contracts: overrides[_contracts], versions: overrides[_versions], group: overrides[_group] }, context, payload, batch);
      }
    };
    ...
    var parseRetrieve = function(callback) {
      return function(err, event_type, context, xhr) {
        ...
        else if (!err) trigger('retrieved', responseJSON, xhr, context);
    ...
    var ClickwrapGroup = function(options) {
      ...
      setParameter(_major_versions, options[_major_versions] || []);
      ...
      setParameter(_force_scroll, options[_force_scroll]);
    ...
    ClickwrapGroup.prototype.extend = function(name, value) {
      this.parameters.extend(name, value);
    };
    
    ClickwrapGroup.prototype.getParams = function(overrides) {
      var self = this;
      overrides = self.getOverrides(overrides);
      return self.site.getParams(overrides);
    };
    
    ClickwrapGroup.prototype.getPayload = function(overrides) {
      var self = this;
      overrides = self.getOverrides(overrides);
      return self.site.getPayload(overrides);
    };
    
    ClickwrapGroup.prototype.getOverrides = function(overrides) {
      var self = this;
      if (!overrides || "object" != typeof overrides) overrides = {};
      ...
      if (self.get(_render_data)) overrides[_render_data] = self.get(_render_data);
    ...
    ClickwrapGroup.prototype.updateHiddenInput = function(val) {
      ...
      setTimeout(_bind(function() {
        trigger('validated', val, { agreed: this.allAgreed(), checked: this.allChecked() }, this);
    ...
    ClickwrapGroup.prototype.injectHTML = function(el) {
      if (el) {
        ...
        var targets = getByClassName(el, 'ps-' + objName + '-target'),
          ...
          inputs = getByClassName(el, 'ps-attr');
        if (this.get(_force_scroll)) this.setupScrollListeners();
        for (var i = 0; i < targets.length; i++) {
          if (targets[i].tagName == 'BUTTON') listenTo(targets[i], 'click', this.handleClick.bind(this), !1);
          else listenTo(targets[i], 'change', this.handleClick.bind(this), !1);
          if (panes && panes.length > 0) listenTo(targets[i], 'change', this.togglePane.bind(this), !1);
        }
        for (var i = 0; i < inputs.length; i++) {
          listenTo(inputs[i], 'change', this.handleInput.bind(this));
          listenTo(inputs[i], 'autofill', this.handleInput.bind(this));
        }
    ...
    ClickwrapGroup.prototype.setupScrollListeners = function() {      
      var self = this,
        el = self.get(_element),
        scrolls = getByClassName(el, 'ps-contract-force-scroll'),
        handler = self.handleScroll.bind(self);
      self.set(_scroll_listener, handler);
      for (var i = 0; i < scrolls.length; i++) listenTo(scrolls[i], 'scroll', handler, !1);
      setTimeout(function() {
        for (var i = 0; i < scrolls.length; i++) handler({ target: scrolls[i] });
      }, 250);
    };
    
    ClickwrapGroup.prototype.handleScroll = function(evt) {
      if (evt && evt.target) {
        var el = this.get(_element),
          target = evt.target,
          bottom = target.offsetHeight + target.scrollTop + target.offsetTop,
          contracts = [].slice.call(getByClassName(target, 'ps-contract-scroll-bottom')),
          tl = getByClassName(el, 'ps-contract-scroll-bottom').length,
          last;
        if (hasClass(target, 'ps-contract-scroll-bottom')) contracts.push(target);
        var cl = contracts.length;
        for (var i = 0; i < contracts.length; i++) {
          last = contracts[i].lastChild;
          if (last && last.offsetParent && bottom >= (last.offsetTop + last.offsetHeight)) {
            cl -= 1;
            tl -= 1;
            this.unblockScroll(contracts[i]);
          }
        }
        if (!tl) this.unblockScroll();
        if (!cl || !tl) stopListening(target, 'scroll', this.get(_scroll_listener), !1);
      }
    };
    
    ClickwrapGroup.prototype.unblockScroll = function(el) {
      if (el) {
        removeClass(el, 'ps-contract-scroll-bottom');
        var cid = getAttribute(el, 'data-cid');
        if (cid) {
          var cel = getElement('ps-contract-' + cid),
            scrollmsg = getElement('ps-scroll-message-' + cid),
            checkbox = getElement('ps-checkbox-clickable-' + cid);
          if (scrollmsg) scrollmsg.style.display = 'none';
          if (checkbox) checkbox.style.display = 'block';
          setAttribute(cel, 'data-scrolled', !0);
          trigger('scrolled:contract', cel, this);
          emit('scrolled:contract', this, cel);
        }
      }
      else {
        el = this.get(_element);
        var scrollmsg = getByClassName(el, 'ps-scroll-message'),
          checkbox = getByClassName(el, 'ps-checkbox-clickable');
        for (var i = 0; i < scrollmsg.length; i++) scrollmsg[i].style.display = 'none';
        for (var i = 0; i < checkbox.length; i++) checkbox[i].style.display = 'block';
        trigger('scrolled', el, this);
        emit('scrolled', this, el);
      }
    };
    ...
    ClickwrapGroup.prototype.retrieveHTML = function(renderData) {
      ...
      xhr.onreadystatechange = function() {
        if (4 == xhr.readyState) {
          try {
            var rdid = this.getResponseHeader('X-Render-ID');
            if (rdid) self.set(_render_id, rdid);
          }
          catch (err) {}
    ...
    ClickwrapGroup.prototype.handleRetrieve = function(err, data, xhr, context) {
      if (data && 'object' == typeof data) {
        var contracts = this.get(_contracts),
          versions = this.get(_major_versions),
            cid,
            i;
        if (!versions || !versions.length) versions = this.get(_versions);
        for (var a in data) {
      ...
      if (!err) trigger('retrieved', data, xhr, context);
          ...
          if (cid.length) {
            for (var c = 0; c < cid.length; c++) {
              if (this.agreed.data.get(cid[c]) === !1) {
                ...
                if (hidden) {
                  setTimeout(_bind(function(_c) {
                    trigger('displayed', _c, this);
                    emit('displayed', this, _c);
                  }, this, cel[i]), 0);
                }
                break;
              }
    ...
    ClickwrapGroup.prototype.handleClick = function(evt) {
      if (evt && evt.target) {
        var self = this,
          ...
          rev = [],
          data = {},
          agreed = evt.target.tagName == 'BUTTON' ? true : evt.target.checked,
        ...
        if (cid.length) {
          ...
          if (self.get(_render_id)) data[_render_id] = self.get(_render_id);
          if (evt.target.tagName == 'BUTTON' && this.get(_element)) {
            var inputs = getByClassName(this.get(_element), 'ps-attr'),
              cus = this.site.get(_custom_data) || {};
            for (var i = 0; i < inputs.length; i++) {cus[inputs[i].getAttribute('name')] = inputs[i].value;}
    ...
    ClickwrapGroup.prototype.togglePane = function(evt) {
      if (evt && evt.target) {
        ...
        if (hasClass(evt.target,'ps-accordion-link')) {
          var contracts = getByClassName(this.get(_element), 'ps-contract');
          for (var i = 0; i < contracts.length; i++) {
            if (contracts[i].dataset.cid == evt.target.dataset.cid) parentEl = contracts[i];
          }
        }
    ...
    ClickwrapGroup.prototype.handleInput = function(evt) {
      if (evt && evt.target) {
        if (hasClass(evt.target, 'ps-attr')) {
          var data = this.site.get(_custom_data);
          if (!data) data = {};
          data[evt.target.getAttribute('name')] = evt.target.value;
          this.site.set(_custom_data, data);
        }
        else this.site.set(_signer_id, evt.target.value);
      }
    };
    ...
    ClickwrapGroup.prototype.allAgreed = function() {
      ...
      if (contracts) {
        for (var i = 0; i < contracts.length; i++) {
          if (!this.agreed.data.get(contracts[i])) {
            all = !1;
            break;
          }
        }
    ...
    BrowsewrapGroup.prototype.render = function(force) {
      ...
      if (!self.rendered || force) {
        if (el) {
          ...
          if (!self.get(_always_visible)) self.setupEvents();
          else {
            var handler = self.updateBadge(self.get(_badge_element));
            self.set(_badge_updater, handler);
    ...
    var commandExecution = function(a) {
      if (isFunction(a[0])) this.method = a[0];
        ...
        if (!this.methodName) throw "command_missing_method";     
          ...
          if (this.args.length < 1 || (typeof this.args[0] != 'string' && typeof this.args[0] != 'object')) throw "command_missing_key";
        }
    ...
    // else if (window.location.hostname == 'app.pactsafe.local') _PactSafe.envAPI = 'response.pactsafe.local:5002';
    // else if (window.location.hostname == 'localhost') _PactSafe.envAPI = 'localhost:3002';
    // else _PactSafe.envAPI = 'dev.pactsafe.io';
    _PactSafe.envAPI = 'response.pactsafe.local:5002';
    ...
    _PactSafe.create = function(a) {
      ...
      if (!_PactSafe.site) _PactSafe.site = site;
      trigger('create', site);
      return site;
    };
    ...
    _PactSafe.enableDebug = function(fn) {
      _PactSafe.disableDebug();
      if (fn && isFunction(fn)) _PactSafe.debug = fn;
      _listenTo('all', isFunction(_PactSafe.debug) ? _PactSafe.debug : logDebug);
      _PactSafe.debugging = !0;
    };
    
    _PactSafe.disableDebug = function() {
      _stopListening('all', isFunction(_PactSafe.debug) ? _PactSafe.debug : logDebug);
      _PactSafe.debugging = !1;
    };
    ...
    _PactSafe.setup = function() {
      ...
      if (!a || 317 != a.realThang) {
        ...
        _PactSafe.loaded = !0;
        _PactSafe.debug = a && a.debug || !1;
        if (_PactSafe.debug) _PactSafe.enableDebug();
        ...
        defineMethod("enableDebug", b, b.enableDebug);
        defineMethod("disableDebug", b, b.disableDebug);
